...
export class BlocksStyleDirective {
  ...
  ngAfterViewInit() {
    ...
    if (this.selector) {
      ...
      if (this.initFirst) {
        // if (this.items[0]) {
        //   (this.items[0] as HTMLElement).setAttribute('style', 'border: 1px solid red; transform: translateY(-1px); box-shadow: 1px 3px 3px rgba(0,0,0,.2)')
        // }
      }
    } else {
  ...
  initKeyUp(ev: KeyboardEvent): void{
    ...
    if (ev.key === 'ArrowRight') {
      ...
      if (this.items[this.index]) {
        (this.items[this.index] as HTMLElement).setAttribute('style', 'border: 1px solid red; transform: translateY(-1px); box-shadow: 1px 3px 3px rgba(0,0,0,.2)')
      }
  ...
  initStyle(index: number) {
    if (this.items[index]) {
      (this.items[index] as HTMLElement).setAttribute('style', 'border: 1px solid red; transform: translateY(-1px); box-shadow: 1px 3px 3px rgba(0,0,0,.2)')
    }
  }
